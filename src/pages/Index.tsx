    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-4 space-y-4">
        ...
        <div className="lg:hidden space-y-4">
          {selectedChannel && (
            <section className="animate-slide-up sticky top-2 z-40">
              {playerSection}
            </section>
          )}
          <section className="space-y-3">
            {searchBar}
            {categoryBar}
            {channelGrid("grid-cols-1 sm:grid-cols-2")}
          </section>
        </div>

        {/* Desktop layout: player left, channel list right + bottom */}
        <div className="hidden lg:block">
          {selectedChannel ? (
            <div className="grid grid-cols-12 gap-4">
              <div className="col-span-8 xl:col-span-9">
                <div className="sticky top-2">{playerSection}</div>
              </div>
              <aside className="col-span-4 xl:col-span-3">
                <div className="sticky top-2 space-y-3 max-h-[calc(100vh-1rem)] overflow-y-auto pr-1">
                  {searchBar}
                  {categoryBar}
                  {channelGrid("grid-cols-1")}
                </div>
              </aside>
            </div>
          ) : (
