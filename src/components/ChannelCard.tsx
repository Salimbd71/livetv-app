...
export function ChannelCard({ channel, isActive, isFavorite, onSelect, onToggleFavorite }: ChannelCardProps) {
  ...
  return (
    ...
    >
      {/* Favorite button — always visible, filled when active */}
      <button
        onClick={onToggleFavorite}
        className={`absolute top-1 right-1 z-10 p-1 rounded-full backdrop-blur-sm transition-all
          ${isFavorite
            ? "bg-primary/20 text-primary hover:bg-primary/30"
            : "bg-background/50 text-muted-foreground/70 hover:text-primary hover:bg-background/80"
          }`}
        data-testid={`btn-fav-${channel.name.replace(/\s+/g, "-")}`}
        title={isFavorite ? "Remove from favorites" : "Add to favorites"}
      >
        <Heart className={`w-3 h-3 md:w-3.5 md:h-3.5 transition-all ${isFavorite ? "fill-current" : ""}`} />
      </button>
      ...
      <div className="aspect-video w-full bg-muted/20 flex items-center justify-center p-2 md:p-4 overflow-hidden relative">
        ...
        <div className={`absolute inset-0 flex items-center justify-center ${channel.logo ? "hidden" : "flex"}`}>
